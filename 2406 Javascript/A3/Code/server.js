...
io.on('connection', function(socket) {
  ...
  socket.on('privateMessage', function(message, recipient){
    let recipients = recipient.split(',').map((r) => r.trim())

    recipients.forEach((r) => {
      const recipientSocket = usernameArr.find((user) => user.username === r)
      if (recipientSocket) {
        io.to(recipientSocket.socketId).emit('serverSays', message, true)
      }
    })
  
    io.to(socket.id).emit('serverSays', message, true)
  })

  socket.on('clientconnect', function(data){
    socket.emit('serverSays', 'You have successfully connected as a user: ' + data)
    //socket.broadcast.emit('serverSays', data)
  })

  socket.on('checkName', function(data, callback){
    let checkForUser = data
    if(usernameArr.some(user => user.username === checkForUser)){
      callback(0)
    }
    else{
      callback(1)
      usernameArr.push({ username: data, socketId: socket.id })
    }
  })

  socket.on('userTaken', function(data){
    socket.emit('serverSays', 'This username is already taken: ' + data)
  })
  ...
  socket.on('disconnect', function(data) {
    ...
    const disconnectedUser = usernameArr.find(user => user.socketId === disconnectedSocket)
    if (disconnectedUser) {
      const index = usernameArr.indexOf(disconnectedUser)
      if (index !== -1) {
