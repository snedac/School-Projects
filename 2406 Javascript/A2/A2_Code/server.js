...
http.createServer(function(request, response) {
  ...
  request.on('end', function() {
    ...
    if (request.method === "POST" && urlObj.pathname === "/solvePuzzle") {
      ...
      let puzzleFile = `puzzles/${dataObj.puzzle.trim()}.txt`
      let solved = dataObj.textArr
      console.log(`Looking for song file: ${puzzleFile}`)
          ...
          fs.readFile(puzzleFile, function(err, data) {
            ...
            if (err) {
              ...
              for(let i = 0; i < fileLines.length; i++){
                let word = fileLines[i]

                for(let j = 0; j < word.length; j++){
                  if(word[j] != solved[track]){
                    check = false
                  }
                  track++
                }
              ...
              response.writeHead(200, { "Content-Type": MIME_TYPES["json"] })
              response.end(JSON.stringify(colour))
            }
          })
        }
        else{
    ...
    else if (request.method === "POST" && urlObj.pathname === "/userText") {
    //a POST request to fetch a song
    //look for song file in songs directory based on song title
